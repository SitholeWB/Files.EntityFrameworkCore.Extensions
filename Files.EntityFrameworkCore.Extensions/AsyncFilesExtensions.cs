...
namespace Files.EntityFrameworkCore.Extensions
{
	public static class AsyncFilesExtensions
	{
		...
		*/
		...
		private static async Task<FilesExtensionsResponse> AddOrSaveFileHelperAsync<T>(DbContext dbContext, Stream stream, string name, string mimeType = "application/octet-stream", Guid? fileId = null, int? chunkSize = null, CancellationToken cancellationToken = default) where T : class, IFileEntity, new()
		{
		...
		{
			return await AddOrSaveFileHelperAsync<T>(dbContext, stream, name, mimeType, fileId, chunkSize, cancellationToken);
		}
		...
		public static async Task<FilesExtensionsResponse> AddFileAsync<T>(this DbContext dbContext, string filePath, Guid? fileId = null, int? chunkSize = null, FileOptions options = FileOptions.None, CancellationToken cancellationToken = default) where T : class, IFileEntity, new()
		{
			...
			return await AddOrSaveFileHelperAsync<T>(dbContext, fileStream, fileInfo.Name, FileHelper.GetMimeType(fileInfo.Extension), fileId, chunkSize, cancellationToken);
		}
